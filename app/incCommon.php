<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'Posts' => "`Posts`.`id` as 'id', IF(    CHAR_LENGTH(`Categoria1`.`Categoria`), CONCAT_WS('',   `Categoria1`.`Categoria`), '') as 'caregoria_id', IF(    CHAR_LENGTH(`Tags1`.`Tag`), CONCAT_WS('',   `Tags1`.`Tag`), '') as 'tag_id', `Posts`.`title` as 'title', `Posts`.`conteudo` as 'conteudo', if(`Posts`.`data`,date_format(`Posts`.`data`,'%m/%d/%Y %h:%i %p'),'') as 'data', `Posts`.`midia` as 'midia'",
			'Categoria' => "`Categoria`.`id` as 'id', `Categoria`.`Categoria` as 'Categoria'",
			'Tags' => "`Tags`.`id` as 'id', `Tags`.`Tag` as 'Tag'",
			'Midia' => "`Midia`.`id` as 'id', `Midia`.`file` as 'file'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'Posts' => "`Posts` LEFT JOIN `Categoria` as Categoria1 ON `Categoria1`.`id`=`Posts`.`caregoria_id` LEFT JOIN `Tags` as Tags1 ON `Tags1`.`id`=`Posts`.`tag_id` ",
			'Categoria' => "`Categoria` ",
			'Tags' => "`Tags` ",
			'Midia' => "`Midia` ",
		];
