<?php
	...
	$transformFunctions = [
		'Posts' => function($data, $options = []) {
			if(isset($data['caregoria_id'])) $data['caregoria_id'] = pkGivenLookupText($data['caregoria_id'], 'Posts', 'caregoria_id');
			if(isset($data['tag_id'])) $data['tag_id'] = pkGivenLookupText($data['tag_id'], 'Posts', 'tag_id');
			if(isset($data['data'])) $data['data'] = guessMySQLDateTime($data['data']);

			return $data;
		},
		'Categoria' => function($data, $options = []) {
	...
	$filterFunctions = [
		...
		'Categoria' => function($data, $options = []) { return true; },
		'Tags' => function($data, $options = []) { return true; },
		'Midia' => function($data, $options = []) { return true; },
	];
